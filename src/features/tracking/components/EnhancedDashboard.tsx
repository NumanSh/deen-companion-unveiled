...
import { ReadingStreakCounter } from '@/features/quran';
  ...
  useEffect(() => {
    const loadRealData = () => {
      ...
      if (dailyTasksData) {
        ...
        completedTasks = tasks.filter((task: unknown) => task.completed).length;
        totalDailyPoints = tasks.filter((task: unknown) => task.completed).reduce((sum: number, task: unknown) => sum + task.points, 0);
      }
